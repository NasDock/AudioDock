...
/** localStorage key：用户主动忽略的活动 ID（同一活动不再弹窗） */
 ...
 *     useCheckPromotion();
 *   useEffect(() => { setTimeout(() => checkPromotion(), 5000); }, []);
 *
 * 弹窗：
 *   <PromotionModal
 *     visible={!!promotion}
 ...
 *     onIgnore={ignorePromotion}   // 勾选忽略后的关闭（写入 localStorage）
 *   />
 */
export const useCheckPromotion = () => {
  ...
  const checkPromotion = async (): Promise<VipCurrentLowestPriceData | null> => {
    ...
    try {
      ...
      // 已被用户忽略 → 不弹
      const ignored = localStorage.getItem(IGNORED_PROMOTION_KEY);
  ...
  /** 忽略当前活动（勾选「忽略此活动」后关闭时调用，写入 localStorage） */
  const ignorePromotion = () => {
    if (promotion?.activityId) {
      localStorage.setItem(IGNORED_PROMOTION_KEY, promotion.activityId);
    }
    setPromotion(null);
  };
